...
function sessionTimer(cssURL1,usertype) {
	...
	this.showExpiredPopup = function() {
		...
		if (sessionWindow!= null && sessionWindow.closed) {
			...
			str += "window.onunload = function () { document.getElementById('gotoLoginImg').trigger('click'); } ";
			str += "</script>";
...
//		window.setTimeout(function () {
		...
		str += "</tr>";
			...
			if(usertype !=4){
				...
				str += "<tr id='rowID3' align='right'>";
				// Changes for TOL 705482 - Continue Session button displaying in session expiry popup for Admin
				str += "<td align='center'><br /><br /><input type='button' id='continue_session_btn' value='Continue Session' onclick='window.close();window.opener.resetSession();' class='btn'></td>";
...
function goToLoginPage(url) {
	// Changes for TOL 953390
    //Calling an event twice to invalidate the session
    document.getElementById("HREF_Logout").click();
    document.getElementById("HREF_Logout").click();
...
function resetSession(){
	var configtimeout = document.getElementById("configSessionTimeout").value;
	if(document.getElementById("MODAL_PREVENT_SESSION_TIMEOUT__")||jQuery("#MODAL_VIEW_CONTAINER").length){
		if(configtimeout && configtimeout!=''){
				resetSessionVar();
		}	
			var dummyLink="";
...
// checkRemarksMandatory method modified for call id : 689814 
 function checkRemarksMandatory(groupletId, formsgroupId) { 
      var id = groupletId+":"+formsgroupId+"."+"_USER_REMARKS_";
   var remarks = feba.domManipulator.getGroupletSpecificElement(formsgroupId+"."+"_USER_REMARKS_", groupletId);
   if(remarks){
    // Changes for TOL 705700
    clearError(id);
    var isMandatory = jQuery(remarks).attr('isMandatory');
    if(isMandatory && isMandatory == 'Y'){
...
// added for call id : 689814 - starts
function clearRemarksError(groupletId, formsgroupId) { 
   var id = groupletId+":"+formsgroupId+"."+"_USER_REMARKS_";
   // Changes for TOL 705700
...
// added for call id : 689814 - ends
/*******************************Added for Account Statemet **********************************************/
function changeReportTitle(groupletId){
	var outFormat = jQuery("select[id='"+groupletId+":TransactionHistoryFG.OUTFORMAT'] option:selected").text();
	//var outFormat = document.getElementById("TransactionHistoryFG.OUTFORMAT").value;
	if(outFormat!=null && outFormat=='PDF')
	                {
	var jspName = "";
	jspName = jQuery("[id='tempReporttitle']").val();
	var reporttitle = "";
	reporttitle = jspName.concat("_");
	var finaltitle = "";
	finaltitle = reporttitle.concat(outFormat);
	jQuery("[id='TransactionHistoryFG.REPORTTITLE']").val(finaltitle);
	}
	else if(outFormat!=null && outFormat=='TXT')
	                {
	var jspName = "";
	jspName = jQuery("[id='tempReporttitle']").val();
	var reporttitle = "";
	...
	else if(outFormat!=null && outFormat=='XLS')
	                {
	var jspName = "";
	jspName = jQuery("[id='tempReporttitle']").val();
	var reporttitle = "";
	...
	var jspName = "";
	jspName = jQuery("[id='tempReporttitle']").val();
	jQuery("[id='TransactionHistoryFG.REPORTTITLE']").val(jspName);
	}	
	}
function changeReportTitleNew(groupletId){

	var outFormat = jQuery("select[id='"+groupletId+":PaymentHistoryListFG.OUTFORMAT'] option:selected").text();
	//var outFormat = document.getElementById("TransactionHistoryFG.OUTFORMAT").value;
   if(outFormat!=null && outFormat=='XLS')
	                {  
	var jspName = "";
	jspName = jQuery("[id='tempReporttitle']").val();
	var reporttitle = "";
	...
	var finaltitle = "";
	finaltitle = reporttitle.concat(outFormat);
	jQuery("[id='PaymentHistoryListFG.REPORTTITLE']").val(finaltitle);
	}
	else{	
	var jspName = "";
	jspName = jQuery("[id='tempReporttitle']").val();
	jQuery("[id='PaymentHistoryListFG.REPORTTITLE']").val(jspName);
	}	
	}
	//*************** Start for captcha on login page is refreshed and not the whole page ***********************
	// Added for call id 937760 
function captchaRefresh() {
		...
		if(document.getElementById('RMAuthenticationFG.VERIFICATION_CODE')!=null){ 
			document.getElementById('RMAuthenticationFG.VERIFICATION_CODE').value="";
		}		
