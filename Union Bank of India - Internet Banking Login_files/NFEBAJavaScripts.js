...
// changes done for Security Token Enh - Call id - 653560 : End 
		// Prafulla_Badgujar Ticket No : 494042 Duplicate inclusion of 'dsprocessor.js' is removed.
			...
			this.include(Constants.FEBA_COMMONJ,'jquery.calendars.islamic.js');		
	     	...
	     	this.include(Constants.FEBA_COMMONJ,'jquery.calendars.lang.js');
	     	this.include(Constants.FEBA_COMMONJ,'jquery.calendars.picker.lang.js');
...
feba.domManipulator.createBaseClass("LIB.Parser",{},{
	...
	parse : function(currentObject,data) {
		...
		function handleNewTransaction(response,content){
            if(response.getResponseHeader(Constants.IS_NEW_TRANSACTION)){
                ...
                // Fix for TOL 713007 - Starts
                // Changes for JS enabled issue when application language changes in preferences update
                if(contentScripts && contentScripts.length){
                	var iNoOfScripts = contentScripts.length;
                	for(var iLoop = 0; iLoop < iNoOfScripts; iLoop++) {
                		if(contentScripts[iLoop].indexOf('JS_ENABLED_FLAG') > -1){
                			executeScriptTag(contentScripts[iLoop]);
                			break;
                		}
                	}
                }
                // Fix for TOL 713007 - Ends
	...
	// Fix for TOL 713007 - Starts
	function executeScriptTag(scriptContent){
		var tempDiv = document.createElement('div');
		tempDiv.innerHTML = scriptContent;
		eval(tempDiv.getElementsByTagName('script')[0].innerHTML);
	}
	// Fix for TOL 713007 - Ends
