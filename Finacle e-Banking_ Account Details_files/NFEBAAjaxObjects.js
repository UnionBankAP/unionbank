...
// changes done for Security Token Enh - Call id - 653560 : Start
				...
				var finParam = createHashandIncrementRia(this.options,requestId);
...
// changes done for Security Token Enh - Call id - 653560 : Start
					...
					beforeSend: function(xhr){xhr.setRequestHeader('requestId', requestId);xhr.setRequestHeader('finParam', finParam);xhr.setRequestHeader('IPTYPE', 'XML');}
// changes done for Security Token Enh - Call id - 653560 : End
...
/* Fix for modal enhancement: end*/
				...
				if(target){					
					...
					LOG.logMessages("View Stack Size in setResponse: "+ currentGroupletOptions.viewStack.length);
					if(currentGroupletOptions.viewStack.length>0){						
...
feba.js.ajax.base.extend("feba.js.ajax.modalView",{},
		...
		execute : function() {
			...
			if(containerDiv.length==0 && this.options.isModalCloseEvent!="Y"){
				...
				var left = ((jQuery(document).width() - this.options.modalDialog.parent().width())/2)+35;
				this.options.modalDialog.parent().css('left',left).css('top',200);
				this.options.modalDialog.css('min-height','');
				
				//Added for fixing pop up in admin corporate snapshot 
				...
				this.options.modalDialog.css('z-index',2000);
...
*/			
			...
			if(!jQuery('#modalDialog input:password').length){
				...
				//Changes done for Call Id 766202 - Added if condition
				if(key != null && key != ""){
			...
			if(userType != '4' && !(typeof isAdaptiveUI != 'undefined' && isAdaptiveUI == "true" )){	
				 feba.domManipulator.documentReady(convertComboboxes);
			}
		...
		handler : function() {
			...
			if(containerDiv.length==0){
				 if(businessError){
						...
						localDM.on('click',localDM.getElementOfClass("modalCloseImg"), function(){
				...
				if (tThis.child.options.showSuccessMessage){
					...
					setTimeout(function(){
						...
						localDM.on('click',localDM.getElementOfClass("modalCloseImg"), function(){tThis.modalCloseAction();});
						}, 500);
				} else {
					localDM.on('click',closeElement, function(){tThis.modalCloseAction();});
					localDM.trigger(closeElement,'click');
				}
