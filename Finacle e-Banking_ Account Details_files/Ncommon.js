	...
	*/
	...
	if(jQuery('#TranRequestManagerFG\\.REPORTTITLE').val() =='AddEntryUX3'){
		...
		if(viewport().width<480){
			...
			if((fsClientBrowser() == 'Safari') && (fsClientBrowser() == 'Safari') && (navigator.userAgent.match(/(iPad|iPhone|iPod)/g)) && navigator.userAgent.match(/like Mac OS X/i)){
				jQuery('.HW_formbtn_search .formbtn_last').css('margin-top','2px');
			}
		}else{
			if(jQuery("#TransactionHistoryFG\\.REPORTTITLE").val()== 'DpMaturityInstructionsUX3'){
...
/*aashish added for touch device issue*/
	jQuery('.mega-menu ul li .sub li.mega-hdr a.mega-hdr-a').on('click',function() {
...
/*Aashish added to simulate long touch event in touch devices end*/
	...
	jQuery(document).on("keypress",function(event){
          ...
          /*Modification for call id 628850 Starts*/
          	//Added condition for call id: 760830 Starts
	        ...
	        if(!(jQuery('#AuthenticationFG\\.REPORTTITLE').val()=='AuthenticationScreen') &&
	        		...
	        		!(jQuery('#RMAuthenticationFG\\.REPORTTITLE').val()=='FBARMAuthenticationScreen')){
	        //Added condition for call id: 760830 Ends	
...
function getAccountChartWidgetByType(typAcct1){
	//Changes for TOL 768303
	jQuery("#RetailUserDashboardUX3_W108__1\\:GO_Event").trigger("click");
...
// File Input Path
jQuery(document).ready(function(){
    jQuery(document).on('change', '.file-field input[type="file"]', function () {
      var file_field = jQuery(this).closest('.file-field');
