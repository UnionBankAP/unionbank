				...
				.autocomplete({
					...
					source: function(request, response) {
						...
						response(select.children("option").map(function() {
							...
							if(comboboxevent !== undefined && comboboxevent.keyCode == 40)
							{
								request.term="";
							}
							/*
							if(window.event == "[object KeyboardEvent]"){
							if(window.event.keyCode != null && window.event.keyCode == 40){
							request.term = "";
							}
							}*/
							/*else
...
that.optionSelected=false;
	return false;
			...
			jQuery(".ui-combobox-input").on("focus",function() {
			    jQuery(this).on('select');
			});
			...
			this.element.on("change.febaCombobox domManipulation.febaCombobox", function(e, trigger) {
		...
		disable: function() {
			...
			this.comboInput.prop("disabled", "true")
...
//					}
			...
			getHiddenDimensions : function(element) {
				if (fsClientBrowser() != 'msie' && (element.width() && element.height())) {
