...
feba.js.custom={
	...
	CorporateApprovalQueueUX3_onload: function(groupletId){
		if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
			...
			jQuery("[id='"+groupletId+Constants.GROUPLET_ELEMENT_SEPERATOR+"SearchPanel_Stage34.SubSectionHeader1']").on('click',
				function (event){
	...
	/*Fix for ticket-735734
	  ...
	  jQuery('.css-labelcheckbox').off('click');
	   ...
	   /*By SHASHI for Demat START*/      
     CustomDematAccountDetails_onload : function(groupletId) {

    	 
    	  	var variable=document.getElementById('DataEntry_LeftContainer_Stage40');
    	  	var variable2=document.getElementById('CustomDematDetailsFG.DEMAT_ACCOUNT');
    	  	var variable3=document.getElementById('DataEntry_LeftContainer_Stage42');
    	  	var variable4=document.getElementById('NavigationPanel_Stage311');
    	  	/*alert(variable+" "+variable2)*/
    	  	if(variable2.value=="STMTACCT")
    	  		{
    	  	...
    	  	if(variable2.value=="DMATSTATUSENQ")
	  		{
    ...
    /*By SHASHI for Demat END*/

    /* START :: Added by Raja for FeedBack*/      
  	CustomFeedbackLink_onload : function(groupletId) {
		//alert("You have reached your maximum limit==");
  		var maxLen = 200;
		var charLeft=document.getElementById('char_left_box');
		//alert("You have reached your maximum limit=="+charLeft.value);
		charLeft.value=maxLen;
		//alert("You have reached your maximum limit=="+charLeft.value);
		/*len=maxLen-count.value.length;
	 	if(len==0)
	 		{
		alert("You have reached your maximum limit=="+len);
	 		}
	 	//alert(document.getElementById('char_left').value);
	 	charLeft.value=len;*/
 	},		
 	/* END :: Added by Raja for FeedBack*/	
	
	  	/*By Sanjeet for PPF*/      
      CustomPPFAccountOpening_onload : function(groupletId) {

  		// Added for hiding sections EBK-100
  		jQuery('[id$="PgHeadingPPF"]').hide();
  		jQuery('[id$="PgHeadingPPF2"]').hide();
  		jQuery('[id$="PgHeadingPPF3"]').hide();
...
//		feba.domManipulator.loadScript("scripts/module/transaction/InitiateFrequencyTypeCheck.js");
	...
	GoalCarDetailsScheduledUX3_onload:function(groupletId){
		jQuery('.stage3_tabmenupanel').last().css('width','100%');	
	},
	GoalHouseDetailsScheduledUX3_onload:function(groupletId){
		jQuery('.stage3_tabmenupanel').last().css('width','100%');	
	},
	GoalOwnDetailsScheduledUX3_onload:function(groupletId){
		jQuery('.stage3_tabmenupanel').last().css('width','100%');	
	},
	GoalRetirementDetailsScheduledUX3_onload:function(groupletId){
		jQuery('.stage3_tabmenupanel').last().css('width','100%');	
	},
	FileUploadProductTypeUX3_onload: function(groupletId){
		jQuery('table >tbody>tr>td').css('background-color','#EBF7FF');
	},
	ApproverListUX3_onload: function(groupletId){
		jQuery('.stage3_tabmenupanel_widoutwidget_topmargin').css('width','100%');
	},
	...
	//for Shopping Mall
	InitiatePayment_onload: function(groupletId){
		feba.js.common.displayWarning = "N";
		feba.domManipulator.loadScript("scripts/module/transaction/InitiateFrequencyTypeCheck.js");
		feba.domManipulator.loadScript("scripts/module/transaction/InitiateCounterpartyTypeCheck.js");
		updateErrorMsgLocation("div","stage3_inputpanel_paydetailsrow");
	...
	/*Added for IMPS Transaction Type :: Start */
	ImmediatePayment_onload: function(groupletId){
	...
	/*Added for IMPS Transaction Type :: End */
	...
	LnTransactionHistoryLstNTxnUX3_onload: function(groupletId) {
		feba.domManipulator.loadScript("scripts/common/NFEBAEllipsis.js");		
		jQuery(".bluelink_wrap").ellipsis({ width : 70});
		var elementId = 'TransactionHistoryFG.OUTFORMAT';
    	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
    		elementId = groupletId+":"+elementId;
    	}
	    var outFormat=document.getElementById(elementId).value; 
		var finalvalue = "";
		document.getElementById(elementId).value= finalvalue;
	},
	LnTransactionHistoryTprUX3_onload: function(groupletId) {
		feba.domManipulator.loadScript("scripts/common/NFEBAEllipsis.js");		
		jQuery(".bluelink_wrap").ellipsis({ width : 70});
		var elementId = 'TransactionHistoryFG.OUTFORMAT';
    	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
	...
	OpTransactionHistoryLstNTxnUX3_onload: function(groupletId) {
		feba.domManipulator.loadScript("scripts/common/NFEBAEllipsis.js");		
		jQuery(".bluelink_wrap").ellipsis({ width : 70});
		var elementId = 'TransactionHistoryFG.OUTFORMAT';
    	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
	...
	OpTransactionHistoryTprUX3_onload: function(groupletId) {
		feba.domManipulator.loadScript("scripts/common/NFEBAEllipsis.js");		
		jQuery(".bluelink_wrap").ellipsis({ width : 70});
		var elementId = 'TransactionHistoryFG.OUTFORMAT';
    	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
	...
	PaymentHistoryListUX3_onload: function(groupletId) {
		var elementId = 'PaymentHistoryListFG.OUTFORMAT';
    	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
    		elementId = groupletId+":"+elementId;
    	}
	    var outFormat=document.getElementById(elementId).value; 
	...
	//Added by Anil for fixing the Style issue START
	CBDTNonTDSPreviewConfirmation_onload: function() {
		jQuery(".HW_tabletextwithpadding").css('background','none');
		jQuery(".HW_tableheadingnobrdr").css('background-color','rgba(0, 0, 0, 0)');
	},
	CBECPreviewConfirmationScreen_onload: function() {
		jQuery(".HW_tabletextwithpadding").css('background','none');
		jQuery(".HW_tableheadingnobrdr").css('background-color','rgba(0, 0, 0, 0)');
	},
	CBDTChallanForm_onload: function() {
		feba.domManipulator.getElement('head').append('<link rel="stylesheet" href="L001/consumer/theme/new_style.css" type="text/css" />');
	},

	//Added by Anil for fixing the Style issue END
	...
	DashboardMailComposeUX3_onload: function(groupletId) { //Modified for TOL : 729846
	var floatDirection  = "right";
		if (feba.domManipulator.isRTL()){
			floatDirection="left";
	}
		jQuery('#'+groupletId).find(".current_tab_widget").css("float",floatDirection); //Modified for TOL : 729846
	...
	MailsListUX3_onload: function(){
		...
		adjustDivHeight();
		 if(viewport().width <640){
			...
			jQuery('#MailsHomeCorp_STATIC_MailListNew\\:Messagetype_Container_Stage35\\.Ra1').css('margin-left','');
...
jQuery( "body" ).on( "click", function(e) {
	...
	if(viewport().width < 480){
			var groupletId = 'PageConfigurationMaster_W45__1\\:RightContainer_Stage3_quicklink';
...
//associate function on browser resize
 ...
 var currChildSmall=jQuery(currContainer).children('.widget-small');
	...
	*/
	...
	/*Added by Anil For Fixing Alignment issue in Tax Pay Authentication Screen -- Start */
	TaxPayAuthenticationScreen_onload: function(){
	...
	/*Added by Anil For Fixing Alignment issue in Tax Pay Authentication Screen -- End */
...
SRRetailFDOpeningDC_onload: function(groupletId){
	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
	 ...
	 var variable= feba.domManipulator.getElementByName('nominee').val();
	 var variable1= feba.domManipulator.getElementByName('minor').val();
		 if (variable=="No")
		 {
			
			  /*document.getElementById("DataEntry_LeftContainer_Stage39.SubSection6").style.display="none";
			  document.getElementById("DataEntry_LeftContainer_Stage39.SubSection7").style.display="none";*/
			    jQuery('[id$="DataEntry_LeftContainer_Stage39.SubSection6"]').hide();
				jQuery('[id$="DataEntry_LeftContainer_Stage39.SubSection7"]').hide();
		 }
		 else if (variable=="Yes")
		 {
			  jQuery('[id$="DataEntry_LeftContainer_Stage39.SubSection6"]').show();
			  if (variable1=="No")
				 {
				jQuery('[id$="DataEntry_LeftContainer_Stage39.SubSection7"]').hide();
				 }
			  else if (variable1=="Yes")
				 {
				  jQuery('[id$="DataEntry_LeftContainer_Stage39.SubSection7"]').show();
				 } else
				 {	
					 jQuery('[id$="DataEntry_LeftContainer_Stage39.SubSection7"]').hide();
				 }
...
PreferencesSetUX3_onload: function(groupletId){
 ...
 // START : For EFRMS -->  
  RSARegisterQnAScreen_onload: function(groupletId){
	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
...
AuthRegisterQnAScreen_onload: function(groupletId){
	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
...
RegisterQnAScreen_onload: function(groupletId){
	if(groupletId && groupletId!=null && groupletId.length>0 && groupletId!="undefined"){
...
if(jQuery('.bubble').hasClass('hideElement')){
	...
	CustomApplyForIPO_onload : function(groupletId) {

		var variable2 = document
				.getElementById('CustomASBAManagementFG.IPO_CAT');
		if (variable2.value == "") {
			jQuery('[id$="PgHeadingCorp"]').hide();
		}

		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
	},

	CustomASBAModifyApplicationDataCapture_onload : function(groupletId) {
		// Added for hiding sections EBK-100
		var variable2 = document
				.getElementById('CustomASBAManagementFG.IPO_CAT');
		if (variable2.value == "") {
	...
	CustomApplyForIPOUnRegDataCapture_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
	...
	CustomConfirmApplyForIPO_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	CustomApplyForIPOCyber_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	CustomApplyForIPOUnRegConfirm_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	
	CustomASBAModifyASBA_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	
	CustomASBARegisterForASBA_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	
	CustomASBAModifyAppCyber_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	CustomASBAModifyApplicationConfirm_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	CustomASBAViewClosedIPODataWithdrawCyber_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
		
	},
	CustomASBAWithdrawApplication_onload : function(groupletId) {
		jQuery('.labelColumn').css('width', '27%');
		jQuery('.labelColumn').attr('style', 'width:27%');
...
PageMaintenanceCreateConfirmationScreen_onload: function(groupletId){
	...
	UpdateChannelUserIdUX3_onload: function(groupletId){
	...
	Authorization_onload: function(){
		...
		/* Changes done for Remarks Mandatory Check */ //added for call id : 689814
	CounterPartyConfirmationDetailsUX3_onload: function(groupletId){
		feba.domManipulator.bind(jQuery('input[type="submit"]'), 'click','',function(event){
			if(event.target.id == groupletId+":SUBMIT"){
				checkRemarksMandatory(groupletId,"CounterPartyCRUDFG");
			} else {
				clearRemarksError(groupletId, "CounterPartyCRUDFG");
			}
		});
	},
	ApprovalBillerRegistrationConfirmationDetailsUX3_onload: function(groupletId){
		feba.domManipulator.bind(jQuery('input[type="submit"]'), 'click','',function(event){
			if(event.target.id == groupletId+":SUBMIT"){
				checkRemarksMandatory(groupletId, "BillerRegistrationCRUDFG");
			} else {
				clearRemarksError(groupletId, "BillerRegistrationCRUDFG");
			}
		});
	},

	ImportLCPreviewConfirmationDetails_onload: function(groupletId){
		feba.domManipulator.bind(jQuery('input[type="submit"]'), 'click','',function(event){
			if(event.target.id == groupletId+":FORM_LC_ISSUANCE.NO_SECTION.REJECT_CONFIRM.PROCESS_FORM_EVENT_AUTH"){
				checkRemarksMandatory(groupletId,"FormManagementFG");
			} else {
				clearRemarksError(groupletId, "FormManagementFG");
			}
		});
	}
	/* Changes done for Remarks Mandatory Check */ //added for call id : 689814
...
user_onload : function(groupletId){
         document.getElementById("HREF_newCaret2").tabIndex = "1";                        
         document.getElementById("PREVENT_SESSION_TIMEOUT__").tabIndex = "2";
         document.getElementById("HREF_caret").tabIndex = "3";
         document.getElementById("updateProfile").tabIndex = "4";
         document.getElementById("updateProfile1").tabIndex = "5";
         document.getElementById("updateProfile2").tabIndex = "6"; 
         document.getElementById("HREF_Logout").tabIndex = "7";
